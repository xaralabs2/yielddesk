...
router.post("/ai/deal-screening", requireAuth, async (req, res): Promise<void> => {
  try {
    ...
    const benchmarkRate = ntbRates.length > 0 ? Math.max(...ntbRates.map(r => parseFloat(String(r.marginalRate ?? "0")))) : 18;
